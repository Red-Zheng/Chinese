...
def main():
    ...
    for entry in data:
        if 'epoch' in entry:
            epoch = entry['epoch']
            epochs.append(epoch)
            s0_loss_bbox.append(entry.get('s0.loss_bbox', 0))
            s0_loss_mask.append(entry.get('s0.loss_mask', 0))
            s1_loss_bbox.append(entry.get('s1.loss_bbox', 0))
            s1_loss_mask.append(entry.get('s1.loss_mask', 0))
            s2_loss_bbox.append(entry.get('s2.loss_bbox', 0))
    ...
    # 左上图：s0.loss_bbox 和 s0.loss_mask
    ...
    # 右上图：s1.loss_bbox 和 s1.loss_mask
    ...
    # 左下图：s2.loss_bbox 和 s2.loss_mask
